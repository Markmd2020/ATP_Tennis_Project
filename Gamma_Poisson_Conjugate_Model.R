...
#Update Aces For Informed and Uninformed Prior
...
#Calculate credible intervals for uninformed prior
n_sim <- 1000
...
#Calculate credible intervals for Informed prior
n_sim <- 1000
shape <- posterior_inf$alpha
rate <-  posterior_inf$beta
lambda_sim <- rgamma(shape,rate)
credible_interval <- quantile(lambda_sim,c(0.025,0.975))
...
for (i in seq_along(player_ids)){
  total_aces[i] <- sum(wimbledon_top8[wimbledon_top8$name==player_ids[i],"ace"])
  total_games[i] <- length(wimbledon_top8[wimbledon_top8$name==player_ids[i],"ace"])
  alpha[i]  <- update_gamma(prior_alpha_inf,prior_beta_inf,total_aces[i],total_games[i])$alpha
  beta[i]  <- update_gamma(prior_alpha_inf,prior_beta_inf,total_aces[i],total_games[i])$beta
}
